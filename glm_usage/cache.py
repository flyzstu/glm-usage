...
Two properties matter for this service:
...
class IntervalGate:
    ...
    ``?refresh=1`` is an explicit bypass of the cache, so on its own it would let
    a two-line shell loop hammer the upstream. This is the rate limit that keeps
    the service inside the site's "collect at most once a minute" guidance.
    """
...
class TTLCache:
    ...
    def age(self, key: Hashable) -> float | None:
        """Seconds since the entry was stored, or ``None`` when there is no entry."""
        entry = self._data.get(key)
    ...
    async def get_or_load(
        ...
        *,
        ...
        ``max_age`` caps how old a value may be, on top of the TTL: it makes both
        a fresh-but-old entry and a stale fallback count as unusable, so callers
        can ask for "nothing older than N seconds" without disabling the cache.
        """
        entry = self._data.get(key)
        now = self._clock()
