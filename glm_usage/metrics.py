...
class Metrics:
    ...
    def record_auth_failure(self) -> None:
        """The upstream turned the token down.

        Sticky until the next successful call, so a token that expires (or gets
        rotated back in) is visible in ``/healthz`` without extra plumbing.
        """
    ...
    def auth_state(self) -> dict[str, Any]:
        """``rejected`` is the latest verdict; ``rejections`` is cumulative."""
        state: dict[str, Any] = {"rejected": self._last_auth_rejection is not None, "rejections": self._auth_rejections}
